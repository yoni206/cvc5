...
**What changed (base work on branch `speedup-compile`; de-hacked on
`unity-dehack`):**
- `src/CMakeLists.txt`: `ENABLE_UNITY_BUILD` (default ON, batch 32) on
  `cvc5-obj`, written as self-documenting config: the batch size is documented
  as the measured sweet spot and overridable via `-DUNITY_BATCH_SIZE`, and the
  files carved out of unity batching sit in four named, commented categories,
  each stating why it must stay a standalone TU:
  1. **Vendored third-party** — `prop/minisat/*` (directory glob).
  2. **Optional SAT-backend wrappers** — `prop/kissat.cpp` /
     `prop/cryptominisat.cpp`, now **gated on `USE_KISSAT` / `USE_CRYPTOMINISAT`**
     (they only collide when the backend header is actually pulled in).
  3. **Generated whole-file-linkage source** — `node_manager.cpp`.
  4. **CoCoA / finite-field** — only under `USE_COCOA`.
- `main/options.cpp` is **no longer compiled into the library at all** (removed
  from `LIBCVC5_SRCS`; it is the driver's `cvc5::main` parser and belongs to the
  `main` target only). This fixes the static-build duplicate-symbol issue at the
  root and removes what used to be a fifth unity carve-out. See "Bug 1" below.
...
original shared-build validation missed it. **Fix (initial):** exclude
`main/options.cpp` from unity, mirroring the `node_manager.cpp` / MiniSat
exclusions. **Fix (final, `unity-dehack`):** don't compile `main/options.cpp`
into the library in the first place — it defines only `cvc5::main::` driver
symbols that no library code references, and the `main` target already compiles
its own copy. Removing it from `LIBCVC5_SRCS` eliminates the duplicate at the
root (nothing to de-dup, in static *or* shared builds) and drops the unity
carve-out entirely. It stays a generated byproduct so the `main` target still
gets it.
